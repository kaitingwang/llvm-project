...
from pydsl.type import UInt32, F64, Index, AnyOp, F32, SInt32
...
def adi(tsteps: Index, n: Index, U: MemrefF32, V: MemrefF32, P: MemrefF32, Q: MemrefF32) -> None:
    b1: F32 = 2.0
    b2: F32 = 1.0
    zero: F32 = 0.0
    dx = b2 / F32(SInt32(n))
    dy = b2 / F32(SInt32(n))
    dt = b2 / F32(SInt32(tsteps))
    mul1 = b1 * dt / (dx * dx)
    mul2 = b2 * dt / (dy * dy)
    ...
    for t in arange(1, tsteps+1):
        ...
        for i in arange(1, n-1):
            ...
            for j in arange(1, n - 1):
                P[i, j] = -c / (a * P[i, j-1] + b)
                Q[i, j] = (-d*U[j, i-1] + (b2 + b1 * d)*U[j, i] - f * U[j, i+1] - a*Q[i, j-1])/(a*P[i, j-1]+b)
    
            V[n - 1, i] = b2
            for j in arange(1, n - 1):
                V[n-j-1, i] = P[i, n-j-1] * V[n-j, i] + Q[i, n-j-1]
        ...
        for i in arange(1, n-1):
            ...
            for j in arange(1, n - 1):
                P[i, j] = -f / (d * P[i, j-1] + e)
                Q[i, j] = (-a*V[i - 1, j] + (b2 + b1 * a)*V[i, j] - c * V[i+1, j] - d*Q[i, j-1])/(d*P[i, j-1]+e)
    
            U[i, n-1] = b2
            for j in arange(1, n - 1):
                U[i, n-j-1] = P[i, n-j-1] * U[i, n-j] + Q[i, n-j-1]
...
if __name__ == "__main__":
    ...
    datasets = {"MINI_DATASET": (20, 20), "SMALL_DATASET": (40, 60), "MEDIUM_DATASET": (100, 200), "LARGE_DATASET": (500, 1000), "EXTRALARGE_DATASET": (1000, 2000)}
    ...
    adi = compile(locals(), transform_seq=transform_seq, dump_mlir=False, auto_build=True, target_class=compilation_target, dataset=current_dataset)(adi)

    if c_test:
        subprocess.run(["gcc-13", "-std=c11", "-O3", "-DPOLYBENCH_TIME", "-DDATA_TYPE_IS_FLOAT", "../utilities/polybench.c", "-shared", f"-D{current_dataset}", "-I../utilities/", "adi.c", "-DNAIVE_VERSION", "-o", "adi-naive.so", "-lm"])
        lib = ctypes.CDLL('./adi-naive.so')
    ...
    if output_array:
        ...
        for i in range(n):
            for j in range(n):
                ...
                print(f"{u[i, j]:.2f} ", end="", file=sys.stderr)
    ...
    if c_test:
        ...
        perf = timeit(lambda: adi_c(tsteps, n, u_ptr, v_ptr, p_ptr, q_ptr), number=1)
        max_difference = 0.0
        for i in range(n):
            for j in range(n):
                max_difference = max(max_difference, abs(u_copy[i, j] - u[i, j]))
        print(perf)
        if max_difference < 0.001:
            ...
            exit(1)
        subprocess.run(["rm", "adi-naive.so"])