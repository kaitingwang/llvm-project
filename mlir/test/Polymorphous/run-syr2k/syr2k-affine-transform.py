...
def syr2k(n: Index, m: Index, alpha: F32, beta: F32, C: MemrefF32NN, A: MemrefF32NM, B: MemrefF32NM) -> None:
    """@tag("distribute")"""
    for i in arange(n):
        for j in arange(i + 1):
        ...
        for k in arange(m):
            for j in arange(i + 1):
                with recursively(lambda x: int_attr(x, "set", 1)):
                    C[i, j] = C[i, j] + alpha * A[i, k] * B[j, k] + alpha * A[j, k] * B[i, k]
...
if __name__ == "__main__":
    ...
    syr2k = compile(locals(), transform_seq=transform_seq, dump_mlir=False, auto_build=True, target_class=compilation_target, dataset=current_dataset)(syr2k)

    if c_test:
        subprocess.run(["gcc-13", "-std=c11", "-O3", "-DPOLYBENCH_TIME", "-DDATA_TYPE_IS_FLOAT", "../utilities/polybench.c", "-shared", f"-D{current_dataset}", "-I../utilities/", "syr2k.c", "-DNAIVE_VERSION", "-o", "syr2k-naive.so", "-lm"])
        lib = ctypes.CDLL('./syr2k-naive.so')
        syr2k_c = lib.kernel_syr2k
    ...
    perf = timeit(lambda: syr2k(n, m, alpha, beta, c, a, b), number=1)
    if output_array:
    ...
    if c_test:
        ...
        perf = timeit(lambda: syr2k_c(n, m, alpha, beta, c_ptr, a_ptr, b_ptr), number=1)
        max_difference = 0.0
        for i in range(n):
            for j in range(n):
        ...
        if max_difference < 0.001:
            ...
            exit(1)
        subprocess.run(["rm", "syr2k-naive.so"])